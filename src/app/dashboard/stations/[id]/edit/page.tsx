...
import { updateStation } from "@/app/actions";
...
export default async function EditStationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [w] = await db.select().from(stations).where(eq(stations.id, stationId));
  if (!w) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8 flex justify-center">
      <Card className="w-full max-w-2xl h-fit">
        <CardHeader>
          <CardTitle>Edit Station: {w.name}</CardTitle>
        </CardHeader>
        <CardContent>
          <form
            action={async (formData) => {
              ...
              await updateStation(stationId, { name, latitude, longitude });
              redirect("/dashboard/stations");
            }}
            className="space-y-4"
          >
